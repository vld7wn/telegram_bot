...
void load_tariff_plans(const std::string &filename)
{
    ...
    try
    {
        ...
        for (const auto &tariff_json : root)
        {
            ...
            if (tariff_json.contains("services") && tariff_json["services"].is_array())
            {
                tariff.services = tariff_json["services"].get<std::vector<std::string>>();
            }
            tariff.extra_details = tariff_json.value("extra_details", "");
...
void save_tariff_plans(const std::string &filename)
{
    ...
    for (const auto &tariff : tariff_plans)
    {
        ...
        t["id"] = tariff.id;
        t["name"] = tariff.name;
        t["mobile_connection_included"] = tariff.mobile_connection_included;
        t["mobile_internet_gb"] = tariff.mobile_internet_gb;
        t["mobile_minutes"] = tariff.mobile_minutes;
        t["mobile_sms"] = tariff.mobile_sms;
        t["tv_kion"] = tariff.tv_kion;
        t["tv_channels"] = tariff.tv_channels;
        t["router_rental"] = tariff.router_rental;
        t["tv_box_rental"] = tariff.tv_box_rental;
        t["connection_fee"] = tariff.connection_fee;
        t["internet_unlimited"] = tariff.internet_unlimited;
        t["internet_limit_gb"] = tariff.internet_limit_gb;
        t["services"] = tariff.services;
        t["extra_details"] = tariff.extra_details;
        t["services"] = tariff.services;

        nlohmann::json speeds = nlohmann::json::array();
        for (const auto &s : tariff.speeds)
        {
            nlohmann::json speed;
            speed["value"] = s.value;
            speed["unit"] = s.unit;
            speed["price"] = s.price;
            speed["promo_price_duration_months"] = s.promo_price_duration_months;
            speed["full_price"] = s.full_price;
    ...
    if (!file.is_open())
    {
        LOG(LogLevel::L_ERROR, "Failed to open tariff plans file for writing: " << filename);
        return;
    }
    file << root.dump(4);
    LOG(LogLevel::INFO, "Tariff plans saved successfully to " << filename);
}
...
void update_tariff_plan(const TariffPlan &tariff)
{
    auto it = std::find_if(tariff_plans.begin(), tariff_plans.end(), [&](const TariffPlan &t)
                           { return t.id == tariff.id; });

    if (it != tariff_plans.end())
    {
        *it = tariff;
    }
    else
    ...
    save_tariff_plans("tariffs.json");
}
...
{
    for (auto &tariff : tariff_plans)
    {
        if (std::find(ids.begin(), ids.end(), tariff.id) != ids.end())
        {
            for (auto &speed : tariff.speeds)
            {
                try
                {
                    int price = std::stoi(speed.price);
                    price = static_cast<int>(price * (1.0 - percent / 100.0));
                    speed.price = std::to_string(price);
                }
                catch (...)
                {
                    LOG(LogLevel::L_WARNING, "Failed to parse price for discount: " << speed.price);
                }
            }
        }
    }
