...
void handle_client_message(TgBot::Bot& bot, TgBot::Message::Ptr message) {
    ...
    if (message->text == "⬅️ Назад") {
        switch (user.state) {
            ...
            case UserState::ENTERING_NAME:
                ...
                break;
            case UserState::ENTERING_PHONE: askForName(bot, chat_id); break;
            case UserState::CHOOSING_MESSENGER: askForPhone(bot, chat_id); break;
            case UserState::ENTERING_EMAIL: askForMessenger(bot, chat_id); break;
            case UserState::ENTERING_CITY: askForEmail(bot, chat_id); break;
            case UserState::ENTERING_STREET: askForCity(bot, chat_id); break;
            case UserState::ENTERING_HOUSE: askForStreet(bot, chat_id); break;
            case UserState::ENTERING_HOUSE_BODY: askForHouse(bot, chat_id); break;
            case UserState::ENTERING_APARTMENT: askForHouseBody(bot, chat_id); break;
            default: sendMainMenu(bot, chat_id); break;
        }
        return;
    }
    ...
    switch (user.state) {
        ...
        case UserState::ENTERING_NAME:
            if (isNotEmpty(message->text)) { user.name = message->text; askForPhone(bot, chat_id); }
            else { bot.getApi().sendMessage(chat_id, "Имя не может быть пустым."); }
            break;
        case UserState::ENTERING_PHONE: {
            std::string phone_number = (message->contact != nullptr) ? message->contact->phoneNumber : message->text;
            if (isValidPhone(phone_number)) { user.phone = phone_number; askForMessenger(bot, chat_id); }
            else { bot.getApi().sendMessage(chat_id, "Неверный формат. Введите 10 цифр или отправьте контакт.\nПример: 912 345 67 89");}
            break;
        }
        case UserState::ENTERING_EMAIL:
            if (isValidEmail(message->text)) { user.email = message->text; askForCity(bot, chat_id); }
            else { bot.getApi().sendMessage(chat_id, "Неверный формат почты (например, user@example.com)."); }
            break;
        case UserState::ENTERING_CITY:
            if (isNotEmpty(message->text)) { user.city = message->text; askForStreet(bot, chat_id); }
            else { bot.getApi().sendMessage(chat_id, "Название города не может быть пустым."); }
            break;
        case UserState::ENTERING_STREET:
            if (isNotEmpty(message->text)) { user.street = message->text; askForHouse(bot, chat_id); }
            else { bot.getApi().sendMessage(chat_id, "Название улицы не может быть пустым."); }
            break;
        case UserState::ENTERING_HOUSE:
            if (isNotEmpty(message->text)) { user.house = message->text; askForHouseBody(bot, chat_id); }
            else { bot.getApi().sendMessage(chat_id, "Номер дома не может быть пустым."); }
            break;
        case UserState::ENTERING_HOUSE_BODY:
            if (message->text != "Пропустить") { user.house_body = message->text; } else { user.house_body = ""; }
            askForApartment(bot, chat_id);
            break;
        case UserState::ENTERING_APARTMENT: {
            if (message->text != "Пропустить") { user.apartment = message->text; } else { user.apartment = "не указана"; }
            ...
            if (user.needs_tv_box && !user.selected_tariff.tv_box_rental.empty()) {
                ...
                if (!rent_details.empty()) rent_details += " + ";
            ...
            if (!user.house_body.empty()) client_invoice << ", корп. " << user.house_body;
            if (user.apartment != "не указана") client_invoice << ", кв. " << user.apartment;
            client_invoice << "\n\n*Расчет стоимости:*\n"
            ...
            if (!user.house_body.empty()) { full_address += ", корп. " + user.house_body; }
            if (user.apartment != "не указана") { full_address += ", кв. " + user.apartment; }
...
void handle_client_callback(TgBot::Bot& bot, TgBot::CallbackQuery::Ptr query) {
    ...
    if (query->data.rfind("show_tariff_detail_", 0) == 0) {
        if (user.state != UserState::CHOOSING_TARIFF) return;
        bot.getApi().answerCallbackQuery(query->id);
    ...
    if (query->data.rfind("select_speed_", 0) == 0) {
        if (user.state != UserState::VIEWING_TARIFF_DETAILS) return;
        bot.getApi().answerCallbackQuery(query->id);
        ...
        if (needs_tv_box_choice) {
            ...
            yes_btn->text = "Да, нужна"; yes_btn->callbackData = "tv_choice_yes";
            auto no_btn = std::make_shared<TgBot::InlineKeyboardButton>();
            no_btn->text = "Нет, спасибо"; no_btn->callbackData = "tv_choice_no";
            row.push_back(yes_btn); row.push_back(no_btn);
    ...
    if (query->data.rfind("tv_choice_", 0) == 0) {
        if (user.state != UserState::CHOOSING_TV) return;
        bot.getApi().answerCallbackQuery(query->id);
    ...
    if (query->data.rfind("messenger_", 0) == 0) {
        if (user.state != UserState::CHOOSING_MESSENGER) return;
        if (query->data == "messenger_telegram") user.preferred_messenger = "Telegram";
        else if (query->data == "messenger_whatsapp") user.preferred_messenger = "What'sApp";
        else if (query->data == "messenger_max") user.preferred_messenger = "MAX";
        bot.getApi().answerCallbackQuery(query->id);
    ...
    if (query->data.rfind("faq_", 0) == 0) {
        if (user.state != UserState::HELP_SECTION) return;
    ...
    if (query->data == "back_to_tariff_list") {
        if (user.state != UserState::VIEWING_TARIFF_DETAILS && user.state != UserState::CHOOSING_TV) return;
        bot.getApi().answerCallbackQuery(query->id);
        sendTariffSelection(bot, chat_id);
...
void sendTradePointSelection(TgBot::Bot& bot, int64_t chat_id) {
    ...
    for (size_t i = 0; i < codes.size(); ++i) {
        ...
        btn->text = codes[i]; btn->callbackData = "flyer_code_" + codes[i];
        row.push_back(btn);
...
void askForPhone(TgBot::Bot& bot, int64_t chat_id) {
    ...
    contact_btn->text = "📱 Отправить мой контакт"; contact_btn->requestContact = true;
...
void askForMessenger(TgBot::Bot& bot, int64_t chat_id) {
    ...
    tg_btn->text = "Телеграм"; tg_btn->callbackData = "messenger_telegram"; messenger_row.push_back(tg_btn);
    auto wa_btn = std::make_shared<TgBot::InlineKeyboardButton>();
    wa_btn->text = "What'sApp"; wa_btn->callbackData = "messenger_whatsapp"; messenger_row.push_back(wa_btn);
    auto max_btn = std::make_shared<TgBot::InlineKeyboardButton>();
    max_btn->text = "MAX"; max_btn->callbackData = "messenger_max"; messenger_row.push_back(max_btn);
    messenger_keyboard->inlineKeyboard.push_back(messenger_row);
    auto back_keyboard = std::make_shared<TgBot::ReplyKeyboardMarkup>();
    auto back_btn = std::make_shared<TgBot::KeyboardButton>();
    back_btn->text = "⬅️ Назад"; back_keyboard->keyboard.push_back({back_btn});
...
void askForEmail(TgBot::Bot& bot, int64_t chat_id) {
    ...
    btn->text = "⬅️ Назад"; keyboard->keyboard.push_back({btn});
    keyboard->resizeKeyboard = true;
...
void askForHouseBody(TgBot::Bot& bot, int64_t chat_id) {
    ...
    auto back_btn = std::make_shared<TgBot::KeyboardButton>();
    back_btn->text = "⬅️ Назад";
    row.push_back(skip_btn); row.push_back(back_btn);
