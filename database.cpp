...
std::string db_get_apps_by_trade_point(const std::string& trade_point_code) {
    ...
    if (sqlite3_prepare_v2(db_main, sql.c_str(), -1, &stmt, 0) == SQLITE_OK) {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            char* cols[10];
            for (int i = 0; i < 10; ++i) { cols[i] = (char*)sqlite3_column_text(stmt, i); }
...
std::string statusToString(ApplicationStatus status) {
    switch (status) {
        case ApplicationStatus::New: return "Новая";
        case ApplicationStatus::InProgress: return "В работе";
        case ApplicationStatus::Done: return "Выполнена";
        case ApplicationStatus::Cancelled: return "Отменена";
...
std::string chatStatusToString(ChatStatus status) {
    switch (status) {
        case ChatStatus::New: return "New";
        case ChatStatus::InProgress: return "In Progress";
        case ChatStatus::Postponed: return "Postponed";
        case ChatStatus::Completed: return "Completed";
